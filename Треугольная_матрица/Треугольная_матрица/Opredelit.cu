...
__host__
void matrix_viewold(float(*array)[SIZE], char *q) {                  //вывод матрицы на экран
	int i, j;
	for (i = 0; i<SIZE; i++) {
		for (j = 0; j<SIZE; j++)
			printf(q, array[i][j]);
...
__host__
void matrix_randold(float(*array)[SIZE]) {               //заполнение случайным образом на девайсе
	int i, j;
	for (i = 0; i<SIZE; i++)
		for (j = 0; j<SIZE; j++)
			array[i][j] = 1 + rand() % 16;
...
__device__
void subrowold(float(*array)[SIZE], int m, int n, float k) {         //вычитание строк на устройстве
...
__global__
void determinantold(float(*mtx)[SIZE]) {             //ядро: приведение к треугольному виду
...
int CudaInfo::Opredelit(void)
{
	...
	determinantold << <numBlock, threadsPerBlock >> >(mtx_d);                      //вызов €дра
